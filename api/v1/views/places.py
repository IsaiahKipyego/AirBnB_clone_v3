...
def get_place_by_city(city_id):
    '''
        Uses Get to return places in city
    '''
...
def get_place_id(place_id):
    '''
 	Uses Get to return places in ID
    '''
...
def delete_place(place_id):
    '''
        DELETEs place object given place_id
    '''
    place = storage.get("Place", place_id)
    if place is None:
        abort(404)
...
def create_place(city_id):
    '''
        creates new place object through city association using POST
    '''
...
def update_place(place_id):
    '''
        updates existing place object using PUT
    '''
    if not request.get_json():
        return jsonify({"error": "Not a JSON"}), 400
